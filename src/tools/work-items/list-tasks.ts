...
export const listTasksSchema = z.object({
  ...
  states: z.array(z.string()).optional().describe("Filter by states (e.g., ['New', 'Active', 'Closed']). If not specified, returns all non-Removed tasks."),
  ...
  inputSchema: {
    ...
    properties: {
      ...
      states: {
        type: "array",
        items: { type: "string" },
        description: "Filter by states (e.g., ['New', 'Active', 'Closed']). If not specified, returns all non-Removed tasks.",
      },
      assignedTo: {
        type: "string",
...
export async function listTasks(
  ...
  const states = validatedParams.states || ["New", "Active", "Closed"];
