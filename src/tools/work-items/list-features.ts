...
export const listFeaturesSchema = z.object({
  ...
  states: z.array(z.string()).optional().describe("Filter by states (e.g., ['New', 'Active', 'Resolved', 'Closed']). If not specified, returns all non-Removed features."),
  ...
  inputSchema: {
    ...
    properties: {
      ...
      states: {
        type: "array",
        items: { type: "string" },
        description: "Filter by states (e.g., ['New', 'Active', 'Resolved', 'Closed']). If not specified, returns all non-Removed features.",
      },
      assignedTo: {
        type: "string",
